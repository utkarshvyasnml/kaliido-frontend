...
export default function RingsSection() {
  ...
  useEffect(() => {
    async function load() {
      const data = await fetchProducts("rings")
      setProducts(data || [])
      setLoading(false)
    }
    load()
  ...
  return (
    <section className="bg-white py-6 md:py-10 overflow-hidden">
      <div className="pl-4 md:pl-10">
        ...
        {/* ================= HEADER ================= */}
        <div className="flex items-start justify-between pr-4 md:pr-10 mb-5">
          <div>
            <h2 className="text-2xl md:text-3xl font-semibold text-black">
              Rings
            </h2>
            <p className="text-sm text-gray-500 mt-1">
              Rings that finish the fit
            </p>
          </div>

          <ViewAllButton href="/rings" />
        </div>
